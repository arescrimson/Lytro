...
module.exports = {
  ...
  async autocomplete(interaction) {
    ...
    const animeNames = animeArray.filter(
      (name) =>
        name.title &&
    ...
    try {
      ...
      collector.on('collect', async (buttonInteraction) => {
        try {
          ...
          const animeCharacterEmbed =
            await animeCharacterSearch.createMainCharacterEmbed(animeObj);
          ...
          collector.on('collect', async (buttonInteraction) => {
            try {
              ...
              if (buttonInteraction.customId === 'right') {
                const updatedEmbed =
                  await animeCharacterSearch.updateCharacterEmbed(true);
                await buttonInteraction
                  .editReply({ embeds: [updatedEmbed] })
                  .catch(console.error);
              } else if (buttonInteraction.customId === 'left') {
                const updatedEmbed =
                  await animeCharacterSearch.updateCharacterEmbed(false);
                await buttonInteraction
                  .editReply({ embeds: [updatedEmbed] })
                  .catch(console.error);
              }
          ...
          await interaction.editReply(
            'Something went wrong with getting main character information.',
          );
      ...
      await interaction.editReply(
        'Something went wrong with getting anime information.',
      );
