...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      if (
        !animeImageEmbed ||
        animeImageSearch.getAnimeImageArrayLength() === 0
      ) {
        interaction.editReply({
          content: 'no images could be found',
          ephemeral: true,
        })
        return
      }
        ...
        collector.on('collect', async (buttonInteraction) => {
          try {
            if (buttonInteraction.customId === 'newImage') {
              const updatedEmbed =
                await animeImageSearch.createAnimeImagesEmbed()
      ...
      await interaction.editReply(
        'something went wrong with getting anime images.',
      )
