...
} = require('../../utils/anime/getCharacter');
  ...
  async autocomplete(interaction) {
    try {
      ...
      await interaction.respond(
        limitedCharacterList.map((names) => ({
          name: names.name,
          value: names.name,
        })),
      );
  ...
  async execute(interaction) {
    try {
      ...
      const animeCharacterEmbed =
        await animeCharacterSearch.createAnimeCharactersEmbed();
      ...
      await interaction.editReply(
        'Something went wrong with getting anime characters.',
      );
