...
class AnimeSearchTests {
  ...
  async testAnimeEmbedValues() {
    test('tests AnimeEmbed values', async () => {
      ...
      expect(testEmbed.data.author.name).toBe(
        'Currently Searching Anime: One Piece',
      )
      expect(testEmbed.data.url).toBe(
  ...
  testGetAnimeRatings() {
    test('getRatings values', () => {
      ...
      expect(validRatings).toBe(
        `Average score from ${300} votes: ${expectedAverage.toFixed(2)} / 10`,
      )
    })
  }
