...
function createAnimeEmbed(
  ...
  return createBaseEmbed()
    ...
    .setURL(`${URL}`)
    .setAuthor({
      name: `Currently Searching Anime: ${TITLE}`,
      iconURL: ICON_URL,
    })
    .addFields(
      { name: 'Synopsis: \n\u200b', value: `${SYNOPSIS}` },
...
function createCharacterEmbed(
  ...
  return createBaseEmbed()
    ...
    .setURL(`${URL}`)
    .setAuthor({
      name: `Currently Searching Character : ${NAME}`,
      iconURL: ICON_URL,
    })
    .addFields(
...
function createAnimeImageEmbed(title, image) {
  return createBaseEmbed()
    .setAuthor({
      name: `Currently Searching Images: ${title}`,
      iconURL: ICON_URL,
    })
    .setThumbnail(THUMBNAIL)
    .setImage(image);
...
function createMangaEmbed(
  ...
  return createBaseEmbed()
    ...
    .setURL(`${URL}`)
    .setAuthor({
      name: `Currently Searching Manga: ${TITLE}`,
      iconURL: ICON_URL,
    })
    .setThumbnail(`${image}`)
    .addFields(
...
function createHelpEmbed() {
  return createBaseEmbed()
    ...
    .addFields(
      ...
      {
        name: '/a [anime_name]',
        value: 'Gets anime information from specified anime.',
      },
      {
        name: '/chr [character_name]',
        value: 'Gets anime character information from specified character.',
      },
      {
        name: '/img [anime_name]',
        value: 'Gets anime image information from specified anime.',
      },
      {
        name: '/rand',
        value: 'Gets anime information from a random anime.',
      },
      { name: '/quote', value: 'Gets a random anime quote.\n\u200b' },
      { name: 'Manga Commands:', value: '\n\u200b' },
      {
        name: '/m [manga_name]',
        value: 'Gets manga information from specified manga.',
      },
      {
        name: '/mrand',
        value: 'Gets manga information from a random manga.\n\u200b',
      },
      { name: 'Game Commands:', value: '\n\u200b' },
      {
        name: '/steam [game_name]',
        value: 'Gets steam game information from specified game.',
      },
      {
        name: '/er [genre] [search]',
        value: 'Gets Elden Ring information about genre searches.\n\u200b',
      },
